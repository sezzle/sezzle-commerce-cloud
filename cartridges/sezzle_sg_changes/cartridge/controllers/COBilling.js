...
function initCreditCardList(cart) {
	...
	applicablePaymentMethods = require('*/cartridge/controllers/Sezzle').Init(cart, applicablePaymentMethods);
...
function publicStart() {
    ...
    if (cart) {
		...
		var applicablePaymentMethods = require('*/cartridge/controllers/Sezzle').Init(cart, creditCardList.ApplicablePaymentMethods);
...
else if (app.getForm('billing').object.paymentMethods.selectedPaymentMethodID.value.equals('Sezzle')) {
        		cart.removePaymentInstruments(cart.getPaymentInstruments(PaymentInstrument.METHOD_CREDIT_CARD));
            		cart.removePaymentInstruments(cart.getPaymentInstruments(PaymentInstrument.METHOD_BML));
          		cart.removePaymentInstruments(cart.getPaymentInstruments('PayPal'));
        		}
